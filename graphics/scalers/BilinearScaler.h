...
template<class uintX, class Manip, class uintS=uintX> class BilinearScaler
{
...
public:
						...
						uint8* pixel, sint32 dw, sint32 dh, sint32 pitch)
	{
		...
		// 2x Scaling
		if ((sw*2 == dw) && (sh*2 == dh))
			return Scale2x(tex,sx,sy,sw,sh,pixel,dw,dh,pitch);
		// 2 X 2.4 Y
		else if ((sw*2 == dw) && (dh*5 == sh*12))
			return ScaleX2Y24(tex,sx,sy,sw,sh,pixel,dw,dh,pitch);
		// 1 X 1.2 Y 
		else if ((sw == dw) && (dh*5 == sh*6))
			return ScaleX1Y12(tex,sx,sy,sw,sh,pixel,dw,dh,pitch);
		// Arbitrary 
		else 
			return ScaleArb(tex,sx,sy,sw,sh,pixel,dw,dh,pitch);
	}
...
#undef ScalePixel2x
#undef X2Xy24xLerps
#undef X2xY24xInnerLoop
#undef X2xY24xDoColsA
#undef X2xY24xDoColsB
#undef X1xY12xCopy
#undef X1xY12xInnerLoop
#undef X1xY12xDoCols
#undef ArbInnerLoop
#undef Read5
#undef Read5_Clipped
